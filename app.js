...
async function startAsCaller(requestId, peerUid, peerHandle) {
  ...
  state.role = "caller";
  state.offerEpoch = 0;
  state.appliedAnswerEpoch = -1;
  state.lastHandledReconnectRequest = 0;
  ...
  // Persistent (not one-shot) listener: reacts to the initial answer AND
  // any later answer produced after we trigger an ICE-restart reconnect.
  onSnapshot(doc(db, "requests", requestId), (snap) => {
    const data = snap.data();
    if (!data) return;
    if (data.answer && data.epoch === state.offerEpoch && state.appliedAnswerEpoch !== data.epoch) {
      pc.setRemoteDescription(new RTCSessionDescription(data.answer));
      state.appliedAnswerEpoch = data.epoch;
    }
    // The other side can request a reconnect even though only the caller
    // can actually create a fresh offer — this flag lets either side's
    // "Reconnect" button work.
    if (data.reconnectRequest && data.reconnectRequest > state.lastHandledReconnectRequest) {
      state.lastHandledReconnectRequest = data.reconnectRequest;
      reconnectAsCaller(state);
    }
  });
...
async function reconnectAsCaller(state) {
  const pc = state.pc;
  ...
  await updateDoc(doc(db, "requests", state.requestId), {
    offer: { type: offer.type, sdp: offer.sdp },
    epoch: state.offerEpoch,
  });
}
...
async function startAsCallee(requestId, peerUid, peerHandle) {
  ...
  // Persistent (not one-shot) listener: reacts to the initial offer AND any
  // later offer sent after the caller performs an ICE-restart reconnect.
  onSnapshot(doc(db, "requests", requestId), async (snap) => {
    const data = snap.data();
    if (!data?.offer) return;
    const epoch = data.epoch ?? 0;
    if (epoch <= state.appliedOfferEpoch) return; // already applied this one
    state.appliedOfferEpoch = epoch;
...
// Callee can't create a fresh WebRTC offer itself (only the original caller
// can) — so its "Reconnect" click just raises a flag the caller is already
// listening for, which then does the actual ICE restart on both's behalf.
async function requestReconnectFromPeer(state) {
  const label = state.card.querySelector(".conn-state");
  if (label) label.textContent = "REQUESTING RECONNECT…";
  await updateDoc(doc(db, "requests", state.requestId), { reconnectRequest: Date.now() }).catch(() => {});
}
...
function markSignalLost(state) {
  ...
  if (!reconnectBtn) {
    ...
    reconnectBtn.addEventListener("click", () => {
      ...
      const attempt = state.role === "caller" ? reconnectAsCaller(state) : requestReconnectFromPeer(state);
