...
async function sendConnectRequest(toUid, toHandle) {
  const reqRef = await addDoc(collection(db, "requests"), {
    ...
    status: "pending",
    createdAt: serverTimestamp(),
    epoch: 0, // bumped on each reconnect/ICE-restart so both sides can tell fresh offers/answers apart from stale ones
  });
...
async function startAsCaller(requestId, peerUid, peerHandle) {
  ...
  const state = createConnectionUI(requestId, peerUid, peerHandle);
  state.role = "caller";
  ...
  onSnapshot(doc(db, "requests", requestId), (snap) => {
    // The signaling doc being deleted is the ONE guaranteed-to-arrive
    // termination signal — it doesn't depend on the data channel being
    // open, unlike a "bye" message, so it's used as the real source of
    // truth for "the other side disconnected."
    if (!snap.exists()) { teardownConnection(requestId, false); return; }
    const data = snap.data();
...
async function startAsCallee(requestId, peerUid, peerHandle) {
  ...
  const unsub = onSnapshot(doc(db, "requests", requestId), async (snap) => {
    ...
    if (data?.offer && !handledOffer) {
      ...
      }).catch(() => {}); // doc may already be gone if peer disconnected mid-handshake
    }
  });
...
// ---------- Reconnect: full wipe + fresh connection ----------
// Completely discards the old RTCPeerConnection and its signaling doc, then
// opens a brand new pipeline from scratch to the same person — simpler and
// far more reliable than trying to resume/ICE-restart the dead one. The
// side that clicks "Reconnect" always becomes the new connection's caller;
// the other side auto-accepts (no popup) via listenForAutoAcceptedRequests.
async function reconnectConnection(state) {
  const { peerUid, peerHandle, requestId: oldRequestId } = state;
  stopHeartbeat(state);
  if (state.dc) state.dc.close();
  if (state.pc) state.pc.close();
  state.card.remove();
  connections.delete(oldRequestId);
  cleanupRequestDoc(oldRequestId); // best-effort, don't block on it
  ...
  const newReqRef = await addDoc(collection(db, "requests"), {
    ...
    toHandle: peerHandle,
    status: "accepted",
    autoAccept: true, // tells the peer to skip the popup and auto-connect
    createdAt: serverTimestamp(),
  });
  await startAsCaller(newReqRef.id, peerUid, peerHandle);
  if (connections.size === 0) emptyState.classList.remove("hidden");
}

// The peer's side of a reconnect: a fresh already-accepted request lands in
// their inbox and is wired up immediately, with no popup.
function listenForAutoAcceptedRequests() {
  const q = query(
    collection(db, "requests"),
    where("toUid", "==", currentUser.uid),
    where("autoAccept", "==", true)
  );
  onSnapshot(q, (snap) => {
    snap.docChanges().forEach((change) => {
      if (change.type === "added") {
        const data = change.doc.data();
        if (!connections.has(change.doc.id)) startAsCallee(change.doc.id, data.fromUid, data.fromHandle);
      }
    });
  });
}
...
function createConnectionUI(requestId, peerUid, peerHandle) {
  ...
  const state = {
    requestId, peerUid, peerHandle, pc: null, dc: null,
    ...
    // heartbeat / reconnect
    signalLost: false,
    lastPingReceived: performance.now(),
    pingSendTimer: null,
    pingCheckTimer: null,
  };
  ...
  card.querySelector(".conn-disconnect").addEventListener("click", () => disconnectAndNotify(state));
...
function updateConnState(state, rtcState) {
  ...
  if (rtcState === "connected") {
    ...
    requestWakeLock();
    if (state.signalLost) markSignalRestored(state);
    ...
    // Only show the reconnect UI if this card hasn't already been
    // intentionally torn down (removed from the connections map).
    if (connections.has(state.requestId)) markSignalLost(state);
  } else {
    dot.classList.add("connecting");
...
function registerDataChannel(state, channel) {
  ...
  channel.onopen = () => { updateConnState(state, "connected"); startHeartbeat(state); };
  channel.onclose = () => { updateConnState(state, "closed"); stopHeartbeat(state); };
...
function handleIncomingMessage(state, data) {
  if (typeof data === "string") {
    ...
    if (msg.type === "meta") {
      ...
      // Peer disconnected on purpose — mirror it here immediately instead
      // of waiting for WebRTC to notice the connection is gone. Don't try
      // to delete the signaling doc again; the initiating side already did.
      teardownConnection(state.requestId, false);
    } else if (msg.type === "ping") {
      state.lastPingReceived = performance.now();
      if (state.signalLost) markSignalRestored(state);
    }
    return;
  }
...
// ---------- Heartbeat ----------
// A tiny "are you still there?" ping travels every 2s in each direction
// while a pipeline is open. If none arrive for 6s straight, we treat the
// link as dead — but unlike an intentional disconnect ("bye"), the card
// stays put and shows a Reconnect button instead of disappearing, since
// this could just be a temporary network blip.
const PING_INTERVAL_MS = 2000;
const PING_TIMEOUT_MS = 6000;

function startHeartbeat(state) {
  stopHeartbeat(state); // guard against double-starting on reconnect
  state.lastPingReceived = performance.now();
  state.pingSendTimer = setInterval(() => {
    if (state.dc && state.dc.readyState === "open") {
      try { state.dc.send(JSON.stringify({ type: "ping" })); } catch { /* ignore */ }
    }
  }, PING_INTERVAL_MS);
  state.pingCheckTimer = setInterval(() => {
    if (!state.signalLost && performance.now() - state.lastPingReceived > PING_TIMEOUT_MS) {
      markSignalLost(state);
    }
  }, PING_INTERVAL_MS);
}

function stopHeartbeat(state) {
  clearInterval(state.pingSendTimer);
  clearInterval(state.pingCheckTimer);
  state.pingSendTimer = null;
  state.pingCheckTimer = null;
}

function markSignalLost(state) {
  if (state.signalLost) return;
  state.signalLost = true;

  const dot = state.card.querySelector(".dot");
  const label = state.card.querySelector(".conn-state");
  dot.classList.remove("connected", "connecting", "online");
  label.textContent = "SIGNAL LOST";

  state.dropzone.style.opacity = "0.5";
  state.dropzone.style.pointerEvents = "none";

  let reconnectBtn = state.card.querySelector(".conn-reconnect");
  if (!reconnectBtn) {
    reconnectBtn = document.createElement("button");
    reconnectBtn.className = "btn-secondary conn-reconnect";
    reconnectBtn.textContent = "Reconnect";
    reconnectBtn.style.marginLeft = "8px";
    reconnectBtn.addEventListener("click", () => {
      reconnectBtn.disabled = true;
      reconnectBtn.textContent = "Reconnecting…";
      reconnectConnection(state);
    });
    state.card.querySelector(".connection-head").appendChild(reconnectBtn);
  }
  reconnectBtn.classList.remove("hidden");
}

function markSignalRestored(state) {
  if (!state.signalLost) return;
  state.signalLost = false;
  state.dropzone.style.opacity = "";
  state.dropzone.style.pointerEvents = "";
  const reconnectBtn = state.card.querySelector(".conn-reconnect");
  if (reconnectBtn) reconnectBtn.classList.add("hidden");
  const label = state.card.querySelector(".conn-state");
  if (label) label.textContent = "SECURE TUNNEL OPEN";
}

// ---------- Disconnect / cleanup ----------
// Sends an explicit "bye" over the data channel (if it's still open) right
// before tearing down, so the other end reflects the disconnect instantly
// instead of waiting for WebRTC's own connectivity-loss detection, which
// can take anywhere from several seconds to tens of seconds.
function disconnectAndNotify(state) {
  if (state.dc && state.dc.readyState === "open") {
    try { state.dc.send(JSON.stringify({ type: "bye" })); } catch { /* channel already going away */ }
  }
  // brief delay so the "bye" message actually has a chance to flush before we close
  setTimeout(() => teardownConnection(state.requestId, true), 150);
}
...
async function teardownAllConnections() {
  for (const state of Array.from(connections.values())) {
    if (state.dc && state.dc.readyState === "open") {
      try { state.dc.send(JSON.stringify({ type: "bye" })); } catch { /* ignore */ }
    }
  }
  await new Promise(r => setTimeout(r, 150)); // let bye messages flush
